    ...
    for r in results:
            ...
            for box in boxes:
                ...
                if class_name == "ok":
                    ...
                    cv2.putText(img, label, (x1, y1 - 2), 0, 1, [0, 255, 0], thickness=5, lineType=cv2.LINE_AA)
                if class_name == "not_ok":
                    cv2.rectangle(img, (x1, y1), (x2, y2), (10, 255, 0), 3)
                    label = f"{class_name} - {conf}"
                    t_size = cv2.getTextSize(label, 0, fontScale=1, thickness=3)[0]
                    c2 = x1 + t_size[0], y1 - t_size[1] - 3
                    cv2.rectangle(img, (x1, y1), c2, [255, 0, 255], -1, cv2.LINE_AA)
...
def main():
    img = capture_frames()
    #img=cv2.imread("C:\\Users\\PMTC-ELE\\Pictures\\IMG_20230721_094600.jpg") 
